...
L.Marker.include({
	...
	_slideProcessing: undefined,

	isSliding: function isSliding() {
		return (this._slideToUntil - performance.now()) < 0;
	},
...
(function(e){var o;if(typeof define==="function"&&define.amd){define(["leaflet"],e)}else if(typeof module!=="undefined"){o=require("leaflet");module.exports=e(o)}else{if(typeof window.L==="undefined"){throw new Error("Leaflet must be loaded first")}e(window.L)}})(function(e){"use strict";e.Circle.SECTIONS_COUNT=64;e.Circle.toPolygon=function(o,t,i){i=i||o._map;if(!i){throw Error("Can't figure out points without adding the feature to the map")}var n=[],r=i.options.crs,f=Math.PI*2,a=0,l,u,d,c,s;if(r===e.CRS.EPSG3857){c=i.latLngToLayerPoint.bind(i);s=i.layerPointToLatLng.bind(i);u=o._radius}else{c=r.projection.project.bind(r.projection);s=r.projection.unproject.bind(r.projection);u=o._mRadius}l=c(o._latlng);t=t||e.Circle.SECTIONS_COUNT;for(var p=0;p<t-1;p++){a-=f/t;d=new e.Point(l.x+u*Math.cos(a),l.y+u*Math.sin(a));if(p>0&&d.equals(n[p-1])){continue}n.push(s(d))}return n};e.Circle.prototype.toPolygon=function(o,t){return e.Circle.toPolygon(this,o,t||this._map)}});
...
function addListField(el, unique_list) {
  ...
  var $panel = $(
    '<div class="panel panel-default"><div class="panel-heading" role="tab" id="listfield' +
      i_listview +
      '"></div><div id="fieldsettings' +
      i_listview +
      '" class="panel-collapse collapse" role="tabpanel" aria-labelledby="listfield' +
      i_listview +
      '"></div></div>'
  );

  if (field.type && numeric_sensors.indexOf(field.type) !== -1) _colors = true;

  var $actions = $('<div class="pull-right"></div>');

  if (_colors)
    $actions.append(
      '<a role="button" data-toggle="collapse" href="#fieldsettings' +
        i_listview +
        '" aria-expanded="false" ><i class="fa fa-cog fa-lg" aria-hidden="true"></i></a> '
    );

  $actions.append(
    '<a role="button" href="javascript:" onClick="removeListField(this, \'.panel\');"><i class="fa fa-times fa-lg" aria-hidden="true"></i></a> '
  );

  $panel.find(".panel-heading").append($actions);
  $panel
    .find(".panel-heading")
    .append('<h4 class="panel-title">' + field.title + "</h4>");
  ...
  if (_colors)
    $body.append(
      '<table class="table"><tr><th>' +
        ...
        "</th><th>" +
        window.lang.color +
        '</th><th><a role="button" href="javascript:" onClick="addListColorField(this, ' +
        i_listview +
...
function addListColorField(el, i) {
  ...
  $row.append(
    '<td><input class="form-control" name="columns[' +
      i +
      "][color][" +
      j_listview +
      '][from]" type="text" value="0"></td>'
  );
  $row.append(
    '<td><input class="form-control" name="columns[' +
      i +
      "][color][" +
      j_listview +
      '][to]" type="text" value="0"></td>'
  );
  $row.append(
    '<td><input class="form-control colorpicker" name="columns[' +
      i +
      "][color][" +
      j_listview +
      '][color]" type="text" value="#000000"></td>'
  );
  $row.append(
    '<td><a role="button" href="javascript:" onClick="removeListField(this, \'tr\');"><i class="fa fa-times fa-lg" aria-hidden="true"></i></a></td>'
  );
...
function HistoryGraph() {
  ...
  _this.loadData = function (type) {
    ...
    const yMin = Math.min(
      ...app.history.positions.map((pos) => pos[sensor.key])
    );
    const yMax = Math.max(
      ...app.history.positions.map((pos) => pos[sensor.key])
    );
    ...
    $("#placeholder").bind("plothover", function (event, pos, item) {
      if (item != null) {
        ...
        $("#hoverdata-date").text(
          moment
            .utc(item.datapoint[0])
            .format(app.settings.dateFormat + " " + app.settings.timeFormat)
        );
      }
    });
    ...
    $("#placeholder").bind("plotclick", function (event, pos, pitem) {
      if (pitem != null) {
        var position_id = pitem.series.data[pitem.dataIndex][3],
...
function HistoryPlayer() {
  ...
  _this.step = function () {
    ...
    let item = app.history.graph.graph_data[options.index],
      position_id = item[3],
      position = app.history.findPosition(position_id);
    ...
    if (options.lastPoint !== null) {
      angle = L.LineUtil.PolylineDecorator.computeAngle(
        options.lastPoint,
        point
      );
    }
    ...
    if (options.marker == null) {
      options.marker = new L.Marker([0, 0], {
        clickable: false,
        keyboard: false,
        icon: L.divIcon({
  ...
  _this.graphSetCrosshair = function (e) {
    var t =
        parseInt(
          app.history.graph.plot.pointOffset({
            x: e,
            y: 0,
          }).left,
          10
        ) - app.history.graph.plot.getPlotOffset().left,
      a = app.history.graph.plot.width(),
      s = parseInt(a / 2, 10);
    t > a - s &&
      app.history.graph.plot.pan({
        left: t - (a - s),
        top: 0,
      }),
      s > t &&
  ...
  _this.graphSetLabel = function (item) {
    ...
    $("#hoverdata-date").text(
      moment.utc(item[0]).format("YYYY-MM-DD HH:mm:ss")
    );
  };
}
...
function History() {
  ...
  _this.events = function () {
    $(document)
      .on(
        "hidden.bs.tab",
        '[data-toggle="tab"][href="#history_tab"]',
        function () {
          _this.clear();
          _this.hideControls();
      ...
      .on("click", "#history-table-content-table tbody tr", function () {
        ...
        var item = $(this).data();
        item.other_arr = jQuery.parseJSON(
          $(this).find(".message_other").html()
        );
        item.sensors_arr = jQuery.parseJSON(
          $(this).find(".message_sensors").html()
        );
  ...
  _this.select = function (history_id) {
    ...
    $('#ajax-history [data-history-id="' + history_id + '"]').addClass(
      "active"
    );
    ...
    _this.polylineSelect.addLayer(
      L.polyline(coordinates, { color: "#66FF33" })
    );
    ...
    setTimeout(function () {
      ...
      if (_bounds) app.map.fitBounds(_bounds, app.getMapPadding());
    }, 100);
  };
  ...
  _this.polylinePointsCheck = function (e) {
    if (_this.polylinePointsSetting) return;
  ...
  _this.polylinePointsSet = function () {
    ...
    if (!app.settings.showHistoryArrow) return;
    ...
    $.each(_this.positions, function (index, position) {
      ...
      _this.polylinePoints.eachLayer(function (arrow) {
        if (10 < point.distanceTo(app.map.project(arrow.getLatLng()))) return;
    ...
    if (window.history_items != null) {
      ...
      $.each(window.history_items, function (index, item) {
        ...
        if (item.status == 2) {
          ...
          if (!app.settings.showHistoryEvent && item.status == 5) return;
        ...
        item.marker = L.marker([item.start.lat, item.start.lng], {
          icon: icon,
  ...
  _this.getParkingIcon = function (item) {
    ...
    return L.divIcon({
      html: icon,
      className:
        "leaflet-marker-icon leaflet-zoom-animated leaflet-interactive leaflet-history-marker",
  ...
  _this.contentPopup = function (item) {
    ...
    if (item.lat && item.lng) {
      parametersHTML +=
        "<tr><th>" +
        window.lang.address +
        ':</th><td><span data-device="address" data-lat="' +
        item.lat +
        '" data-lng="' +
        item.lng +
        '"></span></td></tr>';
      parametersHTML +=
        "<tr><th>" +
        window.lang.street_view +
        ':</th><td><a href="http://maps.google.com/?q=&cbll=' +
        item.lat +
        "," +
        item.lng +
        "&cbp=12,20.09,,0,5&layer=c&hl=" +
        app.lang.iso +
        '" target="_blank">' +
        window.lang.preview +
        " &gt;&gt;</a></td></tr>";
    ...
    $.each(item.metas, function (index, meta) {
      parametersHTML +=
        "<tr><th>" + meta.title + ":</th><td>" + meta.value + "</td></tr>";
    });
  ...
  _this.onDataReceived = function () {
    ...
    $.each(window.history_sensors, function (index, sensor) {
      $graph_sensors.append(
        '<li role="presentation"><a href="#' +
          sensor.key +
          '" role="tab" data-toggle="tab" data-id="' +
          sensor.key +
          '">' +
          sensor.name +
          "</a></li>"
  ...
  _this.popupPointContent = function (item) {
    ...
    parametersHTML +=
      ...
      item.altitude +
      " " +
      app.settings.units.altitude.unit +
      "</td></tr>";
    parametersHTML +=
      "<tr><th>" +
      window.lang.speed +
      ...
      "</td></tr>";
    parametersHTML +=
      "<tr><th>" + window.lang.time + ":</th><td>" + item.time + "</td></tr>";

    if (item.distance_from_start) {
      parametersHTML +=
        "<tr><th>" +
        window.lang.distance +
        " desde el punto inicial:</th><td>" +
    ...
    if (item.duration_from_start) {
      parametersHTML +=
        "<tr><th>" +
        window.lang.duration +
        " desde el punto inicial:</th><td>" +
        item.duration_from_start +
        "</td></tr>";
    }
    ...
    if (typeof item.other_arr != "undefined") {
      parametersHTML +=
        '<div id="history-point-params" class="collapse"><table class="table table-condensed"><tbody>';
      var _other = "";
      ...
      parametersHTML +=
        '<div class="text-center"><i class="btn icon ico-options-h" data-toggle="collapse" data-target="#history-point-params"></i></div>';
      parametersHTML += "</div>";
    }

    var html = "";
    html +=
      '<div class="popup-content" data-history-id="' + item.position_id + '">';
    html +=
      '   <div class="popup-header">' +
      nav +
      '<div class="popup-title"></div></div>';
...
function Devices() {
  ...
  _this.events = function () {
    ...
    $(document)
      .on(
        "hidden.bs.tab",
        '[data-toggle="tab"][href="#objects_tab"]',
        function (e) {
  ...
  _this.hide = function () {
    if (app.widgets && app.widgets.length) app.widgets.hide();
  ...
  _this.follow = function (id) {
    ...
    dialogMoveToTop(
      $("#" + div_id).parent(".ui-dialog.ui-widget.ui-widget-content"),
      true
    );
  };
  ...
  _this.get = function (id) {
    ...
    if (typeof _item === "Device") return null;
  ...
  _this.active = function (id, value) {
    ...
    if (value) {
      if (_device.isLayerVisible()) cluster.addLayer(_device.getLayer());
    } else {
  ...
  _this.activeMulti = function (group_id, value) {
    ...
    $.each(items, function (id, device) {
      ...
      if (device.options().group_id != group_id) return;

      device.setVisible(value);

      if (value) {
        if (device.isLayerVisible()) cluster.addLayer(device.getLayer());
      } else {
  ...
  _this.delete = function (id, confirmed) {
    if (!confirmed) {
      $("#deleteObject button[onclick]").attr(
        "onclick",
        "app.devices.delete(" + id + ", true);"
      );
    ...
    $modal.postData(app.urls.deviceDelete, "DELETE", $("#devices_edit"), {
      id: id,
      _method: "DELETE",
    });
  };
  ...
  _this.remove = function (id) {
    ...
    if (_item.isLayerVisible()) cluster.removeLayer(_item.getLayer());

    _this.refreshCluster();

    if (typeof items[id] !== "undefined") delete items[id];
  };
  ...
  _this.initDeviceIn = function (container, data) {
    ...
    var map = L.map(container, { zoomControl: false, maxZoom: 18 }).setView(
      device.getLatLng(),
      18
    );
  ...
  _this.fitBounds = function (currentZoom) {
    ...
    if (_this.single !== null) {
      ...
      if (_item && _item.isLayerVisible()) _bounds = [_item.getLatLng()];
    }
    ...
    if (_bounds.length) {
      ...
      if (!currentZoom) _option.maxZoom = 18;
  ...
  _this.updateLayers = function () {
    ...
    if (!_animate) cluster.clearLayers();

    if (!app.devices.disableLayer) {
      $.each(items, function (id, device) {
        if (!device) return;

        if (!device.isLayerVisible()) return;
  ...
  _this.refreshCluster = function (_doFitBounds) {
    if (!refreshTimeout) {
      refreshTimeout = setTimeout(function () {
        ...
        if (app.settings.fitBounds || _doFitBounds) _this.fitBounds(true);
  ...
  _this.initCluster = function () {
    ...
    if (cluster) {
      cluster.options.disableClusteringAtZoom = app.settings.clusterDevice
        ? disableZoom
        : 1;
  ...
  _this.clusterPopup = function (cluster) {
    ...
    $.each(cluster.getAllChildMarkers(), function (index, item) {
      ...
      parametersHTML +=
        '<li data-device-id="' +
        id +
        '" onclick="app.devices.get(' +
        id +
        ').onLayerClick();">';
      parametersHTML +=
        '<div class="name">' + device.dataDOM("name") + "</div>";
      parametersHTML +=
        '<div class="details">' +
        device.dataDOM("speed") +
        device.dataDOM("status") +
        "</div>";
      parametersHTML += "</li>";
    });
    parametersHTML += "</ul>";
  ...
  _this.updateStopDuration = function () {
    ...
    var device_id = app.widgets.attr("data-device-id"),
      device = _this.get(device_id);

    if (!device) return;

    $('[data-device="stop_duration"]', app.widgets).replaceWith(
      device.dataDOM("stop_duration")
    );

    if ($popup.length) {
      $('[data-device="stop_duration"]', $popup).replaceWith(
        device.dataDOM("stop_duration")
      );
    }
  };
...
function Pois() {
  ...
  _this.events = function () {
    ...
    $(document).on(
      "change",
      '#pois_export select[name="export_type"]',
      function () {
    ...
    $(document)
      .on(
        "hidden.bs.tab",
        '[data-toggle="tab"][href="#pois_create"]',
        function (e) {
          _this.cancelEditing();
        }
      )
      .on(
        "hidden.bs.tab",
        '[data-toggle="tab"][href="#pois_edit"]',
        function (e) {
          _this.cancelEditing();
        }
      );
  ...
  _this.get = function (id) {
    ...
    if (typeof _item === "Poi") return null;
  ...
  _this.active = function (poi_id, value) {
    ...
    if (value) {
      if (_item.isLayerVisible()) layerGroup.addLayer(_item.getLayer());
    } else {
  ...
  _this.activeMulti = function (group_id, changeItems, value) {
    ...
    $.each(items, function (id, poi) {
      ...
      if (poi.options().group_id != group_id) return;
  ...
  _this.initForm = function (item, $container) {
    let checked = $container.find(
      '.icon-list input[name="map_icon_id"]:checked'
    );
  ...
  _this.edit = function (id) {
    ...
    if (tmpItem.getLatLng()) _this.map().setView(tmpItem.getLatLng());
  ...
  _this.delete = function (id, confirmed) {
    if (!confirmed) {
      $("#deletePoi button[onclick]").attr(
        "onclick",
        "app.pois.delete(" + id + ", true);"
      );
    ...
    $modal.postData(app.urls.poisDelete, "DELETE", $("#pois_edit"), {
      id: id,
      _method: "DELETE",
    });
  };
  ...
  _this.remove = function (id) {
    ...
    if (_item.isLayerVisible()) layerGroup.removeLayer(_item.getLayer());
  ...
  _this.tmpUpdate = function (data) {
    ...
    $('[name="coordinates"]', $container).val(
      tmpItem.getLatLng() === null ? "" : JSON.stringify(tmpItem.getLatLng())
    );
  ...
  _this.showLayers = function () {
    $.each(items, function (id, item) {
      if (_this.canDisplay(item)) layerGroup.addLayer(item.getLayer());
    });
  };
  ...
  _this.checkShowingOnZoomLevel = function () {
    ...
    $.each(items, function (id, poi) {
      if (!_this.canDisplay(poi)) return;

      if (!mapBounds.contains(poi)) return;
      ...
      if (!isMaxZoom) {
        layerGroup.getLayers().some(function (displayedLayer) {
          if (
            point.distanceTo(_this.map().project(displayedLayer.getLatLng())) <
            45
          )
            return (skip = true);
        });
  ...
  _this.canDisplay = function (item) {
    ...
    if (!item.isLayerVisible()) return false;
...
function pois_create_modal_error_callback(res) {
  sidebarAutoHeight();
}
function pois_edit_modal_error_callback(res) {
  sidebarAutoHeight();
}
function pois_import_modal_error_callback(res) {
  sidebarAutoHeight();
}
...
function Geofences() {
  ...
  _this.events = function () {
    ...
    $(document).on(
      "change",
      '#geofences_export select[name="export_type"]',
      function () {
    ...
    $(document)
      .on(
        "hidden.bs.tab",
        '[data-toggle="tab"][href="#geofencing_create"]',
        function (e) {
          _this.cancelEditing();
        }
      )
      .on(
        "hidden.bs.tab",
        '[data-toggle="tab"][href="#geofencing_edit"]',
        function (e) {
          _this.cancelEditing();
        }
      );
    ...
    $(_this.map()).on(
      "geofence.created geofence.updated",
      function (e, geofence) {
        dd("geofence", e, geofence);

        if (!geofence.isLayerVisible()) return;

        var layer = geofence.getLayer();

        if (layer) _this.map().addLayer(layer);
      }
    );
  ...
  _this.get = function (id) {
    ...
    if (typeof _item === "Geofence") return null;
  ...
  _this.activeMulti = function (group_id, value) {
    ...
    $.each(items, function (id, geofence) {
      ...
      if (geofence.options().group_id != group_id) return;
  ...
  _this.toggleLayer = function (item, value) {
    if (value) {
      if (item.isLayerVisible()) _this.map().addLayer(item.getLayer());
    } else {
  ...
  _this.delete = function (id, confirmed, modal_id = "geofencing_edit") {
    if (!confirmed) {
      $("#deleteGeofence button[onclick]").attr(
        "onclick",
        "app.geofences.delete(" + id + ', true, "' + modal_id + '");'
      );
    ...
    $modal.postData(app.urls.geofenceDelete, "DELETE", $("#" + modal_id), {
      id: id,
      _method: "DELETE",
    });
  };
  ...
  _this.remove = function (id) {
    ...
    if (_item.isLayerVisible()) _this.map().removeLayer(_item.getLayer());
  ...
  _this.tmpUpdate = function (data) {
    ...
    $('[name="coordinates"]', $container).val(
      JSON.stringify(tmpItem.getBounds())
    );
...
function Routes() {
  ...
  _this.events = function () {
    ...
    $(document)
      .on(
        "hidden.bs.tab",
        '[data-toggle="tab"][href="#routes_create"]',
        function (e) {
          _this.cancelEditing();
        }
      )
      .on(
        "hidden.bs.tab",
        '[data-toggle="tab"][href="#routes_edit"]',
        function (e) {
          _this.cancelEditing();
        }
      );
    ...
    $(document).on(
      "change",
      '#routes_export select[name="export_type"]',
      function () {
    ...
    $(_this.map()).on("route.created route.updated", function (e, route) {
      ...
      if (!route.isLayerVisible()) return;
  ...
  _this.get = function (id) {
    ...
    if (typeof _item === "Route") return null;
  ...
  _this.active = function (route_id, value) {
    ...
    if (value) {
      if (_item.isLayerVisible()) _this.map().addLayer(_item.getLayer());
    } else {
  ...
  _this.activeMulti = function (group_id, changeItems, value) {
    ...
    $.each(items, function (id, route) {
      ...
      if (route.options().group_id != group_id) return;
  ...
  _this.delete = function (id, confirmed) {
    if (!confirmed) {
      $("#deleteRoute button[onclick]").attr(
        "onclick",
        "app.routes.delete(" + id + ", true);"
      );
    ...
    $modal.postData(app.urls.routeDelete, "DELETE", $("#routes_edit"), {
      id: id,
      _method: "DELETE",
    });
  };
...
function Alerts() {
  ...
  _this.events = function () {
    $(document).on(
      "change ajax.bs.select",
      '.alert-form select[name="devices[]"]',
      function () {
        var $select = $('.alert-form select[name="events_custom[]"]'),
    ...
    $(document).on(
      ...
      '.alert-form select[name="devices[]"]',
      function () {
        var $select = $('.alert-form select[name="command[type]"]'),
          data = {
            alert_id: $('.alert-form input[name="id"]').val(),
            devices: [],
            selected_devices: $('.alert-form input[name="selected_devices[]"]')
    ...
    $(document).on(
      "change",
      '.alert-form select[name="command[type]"]',
      function () {
        var type = $(this).val();
    ...
    $(document).on("change", '.alert-form select[name="type"]', function () {
      ...
      switch ($(this).val()) {
        ...
        case "poi_idle_duration":
          $('a[href="#alerts-form-geofences"]', $modal)
            .addClass("disabled")
        ...
        case "move_duration":
          $('a[href="#alerts-form-geofences"]', $modal)
            .addClass("disabled")
            .css("pointer-events", "none");
          $('a[href="#alerts-form-schedule"]', $modal)
            .addClass("disabled")
            .css("pointer-events", "none");
          break;
        default:
    ...
    $(document).on("hide.bs.modal", ".modal", function (e) {
      if (!$(e.target).find(".alert-form").length) return;
...
function DeviceEvents() {
  ...
  _this.events = function () {
    $(document)
      ...
      .on(
        "shown.bs.tab",
        '[data-toggle="tab"][href="#events_tab"]',
        function (e) {
          _this.list();
        }
      );
  ...
  _this.get = function (id) {
    ...
    if (typeof _item === "EventSys") return null;
...
var app = {
  ...
  loadOn: function (url, container, callback, containerReplace) {
    var $container = $(container),
      containerReplace =
        typeof containerReplace === "undefined" ? true : containerReplace;
    ...
    $.ajax({
      ...
      success: function (response) {
        if (containerReplace) $container.replaceWith(response);
  ...
  check: function (id) {
    $.ajax({
      ...
      success: function (res) {
        if (res.version != app.version) return window.location.reload();
  ...
  initSocket: function () {
    if (location.protocol == "http:")
      ...
      this.socket = io(
        "https://" + document.domain + ":" + this.socket_ssl_port
      );
  ...
  setupWysihtml5: function () {
    ...
    $.fn.wysihtml5.defaultOptions = Object.assign(
        ...
        events: {
          ...
          change_view: function () {
            ...
            this.setValue(
              parseSmsContent(
                this.getValue(),
                typeof this.currentView.selection !== "undefined"
              )
            );
          },
        },
    ...
    switch (setting) {
      ...
      case "showNames":
        if (this.settings.showNames != value) {
          ...
          else $(this.map.getContainer()).addClass("leaflet-hidden-name");
        }
        break;
      case "showTail":
      ...
      case "showPoi":
        if (this.settings.showPoi != value) {
          ...
          if (this.settings.showPoi) this.pois.showLayers();
          else this.pois.hideLayers();
        }
      ...
      case "showGeofences":
        if (this.settings.showGeofences != value) {
          ...
          if (this.settings.showGeofences) this.geofences.showLayers();
          else this.geofences.hideLayers();
        }
      ...
      case "showRoutes":
        if (this.settings.showRoutes != value) {
          ...
          if (this.settings.showRoutes) this.routes.showLayers();
          else this.routes.hideLayers();
        }
      ...
      case "showTraffic":
        ...
        var layer = $('input[name="leaflet-base-layers"]:checked')
          .closest("label")
          .find("span")
          .text()
          .trim();
...
function initComponents(container) {
  ...
  $(".datepicker", $container).datepicker({
    ...
    autoclose: true,
    weekStart:
      typeof app.settings.weekStart !== "undefined"
        ? app.settings.weekStart
        : 0,
    language: app.lang.iso,
  });
  ...
  if (false && $_dataAddress.length) {
    ...
    $_dataAddress.each(function () {
      ...
      if (typeof app.addressStack[_key] !== "undefined") return;
  ...
  $('[data-toggle="collapse"][data-url]', $container).each(function () {
    ...
    $target.off("show.bs.collapse").on("show.bs.collapse", function () {
      $target.html("");
  ...
  $('[data-toggle="scroll"][data-url]', $container).each(function () {
    ...
    var check = function () {
      ...
      if (!$source.is(":visible")) return;

      if (!elementContainerView($scroller, $source)) return;
    ...
    if (!$scroller.length) $scroller = $parent.parent();
    ...
    $parent.off("resize").on("resize", function () {
      clearTimeout(timeout);
      timeout = setTimeout(function () {
        check();
      }, 100);
  ...
  $('[data-toggle="tab"][data-url]', $container).each(function () {
    ...
    $source.off("show.bs.tab").on("show.bs.tab", function () {
      remoteContent($source.attr("data-url"), $target);
    });
  });
  ...
  $(".phone_number").each(function () {
    ...
    $input.on("keyup change countrychange", function () {
      $hidden.val(
        "+" +
          $input.intlTelInput("getSelectedCountryData").dialCode +
          $input.val()
      );
    });
...
function sidebarAutoHeight() {
  ...
  $(".sidebar").each(function () {
    ...
    var height =
      $sidebar.height() - ($tab_content.offset().top - $sidebar.offset().top);
...
function bottomAutoWidth(timeout) {
  setTimeout(
    function () {
      ...
      $(".map-controls.top-left").css(
        "padding-left",
        ($sidebar.is(":visible") ? $sidebar.width() : 0) + "px"
      );
      $(".map-controls.bottom-right").css(
        "padding-bottom",
        ($bottombar.is(":visible") ? $bottombar.height() : 0) + "px"
      );
    },
    timeout ? 460 : 100
  );
...
$(window).on("load", function () {
  ...
  $(document)
    .on(
      "shown.bs.tab",
      '[data-toggle="tab"][href="#gps-device-street-view"]',
      function (e) {
    ...
    .on(
      "shown.bs.tab",
      '[data-toggle="tab"][href="#gps-device-street-view-large"]',
      function (e) {
  ...
  $(document).on("focus", ".empty-input-add-new", function () {
    ...
    copy.find(".bootstrap-select").replaceWith(function () {
      return $("select", this);
    });
  ...
  $(document).on("click", "#showPoint .btn-action", function () {
    ...
    var lat = $(
      ".cor_type_" + type + ' input[name="latitude"]',
      $container
    ).val();
    var lng = $(
      ".cor_type_" + type + ' input[name="longitude"]',
      $container
    ).val();
    var latlng;
  ...
  $(document).on("change", 'select[name="engine_hours"]', function () {
    var val = $(this).val();
    if (val === "engine_hours") $(".ignition_detection_engine").show();
    else $(".ignition_detection_engine").hide();
  });
...
function Commands() {
  ...
  _this.setValues = function (values) {
    ...
    if (values) current_type = values.type;
  };

  _this.getDeviceCommands = function (
    data,
    onBeforeSend,
    onComplete,
    commandsUrl
    ...
    if (!commandsUrl) commandsUrl = app.urls.devicesCommands;
  ...
  _this.buildTypesSelect = function ($select) {
    ...
    $.each(commandList, function (index, command) {
      ...
      $select.append(
        '<option value="' + command.type + '">' + command.title + "</option>"
      );
    });

    if (
      current_type != null &&
      $('option[value="' + current_type + '"]', $select).length
    ) {
      $select.val(current_type);
    }

    $select.trigger("change").selectpicker("refresh");
  };
...
function DeviceMedia() {
  ...
  _this.currentDeviceMedia = function () {
    var url = (device_id = $("#widgets").attr("data-device-id"));

    return $modal.getModalContent(
      {
        url:
          "./device_media/create" + device_id
            ? "device_id=" + $("#widgets").attr("data-device-id")
            : "",
      },
      "GET",
      $modal.initModal("camera_photos")
    );
  };
  ...
  _this.resetCameraWindow = function () {
    ...
    $("#camera_photos .alert-danger.main-alert")
      .html("")
      .css("display", "none");
...
function Device(data) {
  ...
  _this.updateLayer = function () {
    ...
    let speed = app.settings.mapShowDeviceSpeed
...
function Poi(data, map) {
  ...
  _this.isLayerVisible = function () {
    ...
    if (!options.coordinates) return false;

    if (typeof options.coordinates.lat === "undefined") return false;

    if (typeof options.coordinates.lng === "undefined") return false;
  ...
  _this.getLayer = function () {
    ...
    var icon_url = options.map_icon.url,
    ...
    if (layer) {
      layer.setIcon(poiIcon).setLatLng(position);
  ...
  _this.openPopup = function () {
    ...
    parametersHTML += '<table class="table table-condensed"><tbody>';
    parametersHTML +=
      "<tr><th>Description:</th><td>" + options.description + "</td></tr>";
    parametersHTML += "</tbody></table>";
...
function Geofence(data, map) {
  ...
  _this.setLayer = function (_layer) {
    ...
    layer.on("remove", _this.onLayerRemove).on("add", _this.onLayerAdd);
  ...
  _this.addPopup = function () {
    ...
    if (!app.settings.showGeofences) return;

    var size = app.settings.showGeofenceSize
      ? _this.formatArea(_this.getArea())
      : null;

    popup = new L.Marker(null, {
      icon: L.divIcon({
        html:
          '<div class="name" style="background-color: ' +
          convertHex(options.polygon_color, 81) +
          '">' +
          options.name +
          (size ? " (" + size + ")" : "") +
          "</div>",
  ...
  _this.polygonToCircle = function () {
    if (!options.coordinates) return;

    if (!options.coordinates.length > 0) return;

    var _layer = L.polygon(options.coordinates);

    _map.addLayer(_layer);

    if (!_layer.getBounds().isValid()) return;

    var center = _layer.getBounds().getCenter(),
      radius =
        _layer
          .getBounds()
          .getSouthWest()
          .distanceTo(_layer.getBounds().getNorthEast()) / 2;
  ...
  _this.circleToPolygon = function () {
    ...
    var coordinates = _layer.toPolygon();
...
function Route(data, map) {
  ...
  _this.addPopup = function () {
    ...
    if (!app.settings.showRoutes) return;

    popup = new L.Marker(null, {
      icon: L.divIcon({
        html:
          '<div class="name" style="background-color: ' +
          convertHex(options.color, 81) +
          '">' +
          options.name +
          "</div>",
...
function EventSys(data) {
  ...
  _this.notice = function () {
    ...
    if (options.geofence != null) {
      if (options.geofence.name != null) {
        ...
        notificationMessage +=
          window.lang.geofence + ": " + options.geofence.name + ",";
      }
    }
    ...
    if (options.device != null) {
      ...
      notificationMessage +=
        window.lang.device + ": " + options.device.name + ", ";
    }
    ...
    notificationMessage = notificationMessage.slice(
      0,
    ...
    let delay =
      (typeof options.delay !== "undefined" ? options.delay : 5) * 1000;
  ...
  _this.html = function () {
    ...
    var _message =
      options.message +
      (options.geofence != null ? " (" + options.geofence.name + ")" : "");

    var $dropdown = $(
      '<div class="btn-group dropleft droparrow"  data-position="fixed"><i class="btn icon options" data-toggle="dropdown" data-position="fixed" aria-haspopup="true" aria-expanded="false"></i></div>'
    );
    ...
    $dropdownMenu.append(
      '<li><a href="javascript:;" data-url="' +
        ...
        "/" +
        options.alert_id +
        '" data-modal="alerts_edit"> <span class="icon event"></span><span class="text">' +
        window.lang.alert +
        "</span></a></li>"
    );
    $dropdownMenu.append(
      '<li><a href="javascript:;" data-url="' +
        app.urls.eventDoDelete +
        "?id=" +
        options.id +
        '" data-modal="events_do_destroy"> <span class="icon delete"></span><span class="text">' +
        window.lang.remove +
        "</span></a></li>"
    );
    ...
    _html +=
      '<span class="time">' +
      moment.utc(options.time).format(app.settings.timeFormat) +
      "</span>";
    _html +=
      '<span class="date">' +
      moment.utc(options.time).format(app.settings.dateFormat) +
      "</span>";
    _html += "</div>";
    ...
    if (app.settings.showEventSectionAddress) {
      ...
      _html +=
        '<span data-device="address" data-lat="' +
        options.latitude +
        '" data-lng="' +
        options.longitude +
        '"></span>';
...
function MapTiles() {
    ...
    _maps = {
      ...
      10: {
        ...
        tile: function () {
          ...
          var tilesLink =
            "https://maps.hereapi.com/v3/{resource}/{projection}/{z}/{x}/{y}/{format}?apiKey={apiKey}&style={style}&size={size}&lang={lang}";
          ...
          var _tileLayer = L.tileLayer(tilesLink, {
            attribution:
              'Map &copy; 2025 <a href="http://developer.here.com">HERE</a>',
            resource: "base",
            projection: "mc",
        ...
        tile: function () {
          var _tileLayer = new L.TileLayer(
              ...
              app.lang.locale,
            {
              ...
              minZoom: 0,
              attribution:
                '<a href="https://google.com" target="_system">Google Maps</a>',
            }
          );
      ...
      56: {
        name: "Google Map - Trafico Hibrido ≠",
        maxZoom: 21,
        tile: function () {
          var _tileLayer = new L.TileLayer(
            "https://mt0.google.com/vt/lyrs=y,traffic&hl=" +
              app.lang.locale +
              "&x={x}&y={y}&z={z}&s=Ga&gl=" +
              app.lang.locale,
            {
  ...
  _this.initControls = function () {
    ...
    $(layersControl.onAdd(_this.map)).insertBefore(
      $("#map-controls-tile > div").first()
    );
  };
...
function Dashboard() {
  ...
  _this.loadBlockContent = function (name, withoutLoader) {
    ...
    var block = $("#dashboard_blocks").find($("#block_" + name)),
    ...
    $.ajax({
      ...
      beforeSend: function () {
        if (!withoutLoader) loader.add(block_body);
      },
      data: {
        name: name,
      },
